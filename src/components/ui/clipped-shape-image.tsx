  ...
  ({ mediaItems, className, ...props }, ref) => {
    ...
    return (
      <>
        ...
        <section
          ref={ref}
          className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 lg:gap-16 pt-8 ${className || ''}`}
          {...props}
        >
          {itemsToRender.map((item, index) => (
            <figure key={index} className="relative group overflow-hidden shadow-xl" style={{ clipPath: `url(#${item.clipId})` }}>
              {item.type === 'image' ? (
                <img
                  src={item.src}
                  alt={item.title || ''}
                  className="transition-transform duration-500 aspect-[4/5] min-h-full align-bottom object-cover w-full scale-100 group-hover:scale-105"
                />
              ) : (
                <video
                  autoPlay
              ...
              {/* Overlay for text */}
              <div className="absolute inset-0 bg-gradient-to-t from-midnight via-midnight/30 to-transparent flex items-end p-6 md:p-8 opacity-90 transition-opacity duration-300">
                <h3 className="text-white font-serif text-xl md:text-2xl font-bold tracking-wide drop-shadow-lg leading-tight w-full text-left">
                  {item.title}
                </h3>
              </div>
            </figure>
